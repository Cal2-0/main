...
def bacons():
    ...
    if request.method == "POST":
        ...
        if action == "encode":
            ...
            for ch in text:
                if ch in bacon_dict:
                    encoded.cip_bpend(bacon_dict[ch])
                elif ch == ' ':
                    encoded.cip_bpend(' ')  # preserve spaces
                else:
...
@cip_bp.route("/ceaser.html", methods=["GET", "POST"])
def ceaser():
    output = ""
    if request.method == "POST":
        text = request.form.get("text", "")
...
def vignere():
    ...
    if request.method == "POST":
        ...
        l = {chr(i + ord('a')): i for i in range(26)}
        rev = {v: k for k, v in l.items()}  # reverse mcip_bping
